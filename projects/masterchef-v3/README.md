...
```solidity
function add(uint256 _allocPoint, IVoltageV3Pool _v3Pool, bool _withUpdate) external
```
...
| _v3Pool | IVoltageV3Pool | Address of the V3 pool. |
...
```solidity
event AddPool(uint256 pid, uint256 allocPoint, IVoltageV3Pool v3Pool, ILMPool lmPool)
```
